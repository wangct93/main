...
DomElement.prototype = {
    ...
    drag: function drag() {
        ...
        return this.data('dragOption', opt).off('mousedown').mousedown(function (e) {
            var $this = $(this);
            var opt = $this.data('dragOption');
            ...
            function mousemove(e) {
                ...
                util.execFunc.call(this, opt.onMousemove, e);
            }
            function mouseup(e) {
                $('body').off('mousemove', mousemove).off('mouseup', mouseup);
                util.execFunc.call(this, opt.onMouseup, e);
            }
            $('body').mousemove(mousemove).mouseup(mouseup);
            util.execFunc.call(this, opt.onMousedown, e);
        });
    },
    ...
    zoom: function zoom() {
        ...
        if (util.isString(opt)) {
            var scale = opt === 'enlarge' ? 1.2 : 1 / 1.2;
            this.forEach(function (elem) {
                var _$$getRect = $(elem).getRect(),
                    width = _$$getRect.width,
                    height = _$$getRect.height;

                zoomFunc(_this4, scale, width / 2, height / 2);
            });
        } else {
            this.data('zoomOption', opt).off('wheel').wheel(function (e) {
                var $this = $(this);
                var opt = $this.data('zoomOption');

                var _$this$getRect = $this.getRect(),
                    top = _$this$getRect.top,
                    left = _$this$getRect.left;

                var dx = e.clientX - left;
                var dy = e.clientY - top;
                var scale = (e.wheelDelta === undefined ? e.deltaY < 0 : e.wheelDelta > 0) ? 1.2 : 1 / 1.2;
                zoomFunc($this, scale, dx, dy);
                $.preventDefault(e);
                util.execFunc.call(this, opt.onWheel, e);
            });
        }
        return this;
    },
        ...
        this.off('paste').on('paste', function (e) {
            ...
            if (item && item.type.indexOf('image') !== -1) {
                ...
                reader.onload = function (e) {
                    util.execFunc.call(target, success, e.target.result);
                };
                reader.readAsDataURL(file);
            } else {
                util.execFunc.call(target, success, clipboardData.getData('text'));
            }
        ...
        this.off('drop').on('drop', function (e) {
            ...
            var q = new util.Queue({
                ...
                success: function success() {
                    util.execFunc.call(target, _success, this.result);
                }
            });
            q.start();
...
var zoomFunc = function zoomFunc($target, scale, dx, dy) {
    var _$target$getRect = $target.getRect(),
        top = _$target$getRect.top,
        left = _$target$getRect.left,
        width = _$target$getRect.width,
        height = _$target$getRect.height;

    var $parent = $target.parent();
    while ($parent.filter('body').length === 0 && $parent.css('position') === 'static') {
        $parent = $parent.parent();
    }

    var _$parent$getRect = $parent.getRect(),
        parentLeft = _$parent$getRect.left,
        parentTop = _$parent$getRect.top;

    var oLeft = left - $target.css('marginLeft').toNum() - parentLeft;
    var oTop = top - $target.css('marginTop').toNum() - parentTop;
    $target.css({
        width: width * scale + 'px',
        height: height * scale + 'px',
...
},{"wt-util":4}],3:[function(require,module,exports){
'use strict';
...
function formData(opt) {
    ...
    $(list).forEach(function (elem) {
        var $elem = $(elem);
        var fieldValue = $elem.attr(field);
        if (fieldValue) {
            ...
            if (data) {
                $elem[eleOpFunc](formatFunc(data[fieldValue]));
            } else {
                result[fieldValue] = $elem[eleOpFunc]();
            }
        }
    });
    return result;
...
},{"./$":2,"wt-util":4}],4:[function(require,module,exports){
(function (global){
'use strict';
...
function addMethods() {
    ...
    forEach(targets, function (copyTarget, name) {
        var globalTarget = win[name];
        if (globalTarget) {
            try {
                for (var _name in copyTarget) {
                    var _prototype = globalTarget.prototype;
                    if (copyTarget.hasOwnProperty(_name) && copyTarget[_name] !== undefined && _prototype[_name] === undefined) {
                        try {
                            Object.defineProperty(_prototype, _name, {
                                value: copyTarget[_name],
                                enumerable: false
                            });
                        } catch (e) {
                            _prototype[_name] = copyTarget[_name];
                        }
                    }
                }
            } catch (e) {
...
function clone(obj, bol, filterNames) {
    ...
    if (isPlainObj(obj) || isArray(obj)) {
        target = isArray(obj) ? [] : {};
        var setValue = function setValue(target, data, name, bol) {
            var value = data[name];
...
function equal(first, second) {
    if ((typeof first === 'undefined' ? 'undefined' : _typeof(first)) === (typeof second === 'undefined' ? 'undefined' : _typeof(second))) {
        if (isPlainObj(first) || isArray(first)) {
            ...
            for (var _name2 in second) {
                if (second.hasOwnProperty(_name2)) {
                    if (!fieldData[_name2] && !equal(first[_name2], second[_name2])) {
                        return false;
                    }
                }
            }
...
var Cache = function () {
    ...
    Cache.prototype.setOption = function setOption(option) {
        this.props = extend(this.props || {}, option);
    };

    Cache.prototype.add = function add(items) {
        this.doMain(items, '_addItem');
    };

    Cache.prototype.remove = function remove(items) {
        this.doMain(items, '_removeItem');
    };
    ...
    Cache.prototype._addItem = function _addItem(item) {
        var list = getValue(this.props, 'list', []);
        list.push(item);
        var _props = this.props,
            limit = _props.limit,
            _props$field = _props.field,
            field = _props$field === undefined ? 'id' : _props$field;

        var data = getValue(this.props, 'data', {});
        item._id = item[field] || +new Date();
        data[item._id] = item;
    ...
    Cache.prototype._removeItem = function _removeItem(item) {
        var list = getValue(this.props, 'list', []);
        var data = getValue(this.props, 'data', {});
        list.remove(item);
    ...
    Cache.prototype.doMain = function doMain(data, funcName) {
        var _this3 = this;
        ...
        data.forEach(function (item) {
            if (isFunction(_this3[funcName])) {
                _this3[funcName](item);
            }
        });
    };
...
},{"./object/Array":5,"./object/Date":6,"./object/String":7}],5:[function(require,module,exports){
"use strict";
